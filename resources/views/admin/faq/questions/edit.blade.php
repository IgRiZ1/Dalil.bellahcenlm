    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3">FAQ Vraag Bewerken</h1>
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.faq.questions.update', $faqQuestion) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('faq_category_id') is-invalid @enderror" 
                                ...
                                @foreach($faqCategories as $category)
                                    <option value="{{ $category->id }}" {{ old('faq_category_id', $faqQuestion->faq_category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('question') is-invalid @enderror" 
                                   id="question" name="question" value="{{ old('question', $faqQuestion->question) }}" required>
                            @error('question')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('answer') is-invalid @enderror" 
                                      id="answer" name="answer" rows="5" required>{{ old('answer', $faqQuestion->answer) }}</textarea>
                            @error('answer')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
